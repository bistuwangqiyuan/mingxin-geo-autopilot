...
# 1. 实体识别词表
# ---------------------------------------------------------------------------
# 我方品牌别名（大小写不敏感匹配；含产品型号与主体名）
BRAND = "中科存储"
BRAND_ALIASES = [
    "中科存储", "ZK-Storage", "ZK Storage", "ZKStorage",
    "WS7000", "WS5000", "WS-HBMM5000", "WS HBMM5000",
...
COMPETITORS = {
    ...
    "中科曙光 ParaStor": ["曙光", "ParaStor", "中科曙光"],
...
# 我方当前站外信源覆盖（实事求是：基线几乎为零，仅有独立官网）。
# 取值 0–1：0=完全无覆盖，1=高质量持续覆盖。用于覆盖缺口分析的"现状"。
CURRENT_SOURCE_COVERAGE = {
    "独立官网": 0.6,          # 已部署，但缺 GEO 基础设施（schema/llms.txt/robots）
    "CSDN": 0.0, "知乎技术区": 0.0, "GitHub/GitCode": 0.0, "技术白皮书": 0.3,
    ...
    "百科": 0.0, "搜狐号": 0.0, "网易号": 0.0, "微信公众号": 0.0, "微信读书": 0.0,
...
TIERS = {
    "T1": "最窄·最可防御（KV-Cache 卸载/存算分离全闪加速一体机）",
    "T2": "中口径（国产/信创 AI 算力中心存储加速、提升 token 产出的存储方案）",
    "T3": "宽口径（AI 算力中心存储 / AI 存储）",
}

# 复现随机种子（与商业计划 results.json 同源风格）
SEED = 20260621
