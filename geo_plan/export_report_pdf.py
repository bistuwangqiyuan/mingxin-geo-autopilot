# -*- coding: utf-8 -*-
"""把 outputs/中科存储-GEO提升计划.html 经 Playwright(Chromium) 打印为 A4 PDF。

复用 company_profile/export_profile_pdf.py 的苹果风格页眉页脚与打印设置。
输出：根目录 中科存储-GEO提升计划与基线报告.pdf。
...
HTML = os.path.join(BASE, "outputs", "中科存储-GEO提升计划.html")
META = os.path.join(BASE, "outputs", "report_meta.json")
OUT_PDF = os.path.join(os.path.dirname(BASE), "中科存储-GEO提升计划与基线报告.pdf")
...
def _meta():
    try:
        ...
        return {"header": "中科存储 · GEO 提升计划", "footer": "中科存储 ZK-Storage"}
